...
/**
 *
 * @author Искрич Андрей В. <AVIskrich@gmail.com>
 */
public class Fact {
    ...
    public Fact(int id, int colId, String contentType, InputStream content, String difficultie) {
        this.id = id;
        this.colId = colId;
    ...
    public static List<Fact> getFactByCollectionID(int collId, Connection conn){
           try {
               List<Fact> facts = new ArrayList<Fact>();
               PreparedStatement statement = conn.prepareStatement(
                       "select f.ID," +
    ...
    public static void insertFactIntoDB(Fact fact, Connection conn) {
        try {
            PreparedStatement statement = conn.prepareStatement(
                    " insert into app.FACT (ID,COLLID,CONTENT_TYPE,CONTENT,DIFFICULTIE) "
                    + "values(?,?,?,?,?) ");
